...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if not torch.distributed.is_initialized():
            ...
            # Set NCCL timeout to avoid hanging (default is 30 minutes, but we set it to 10 minutes)
            timeout = os.environ.get("NCCL_TIMEOUT", "600")  # 10 minutes default
            init_timeout = timedelta(seconds=int(timeout))
    ...
    def _build_model_optimizer(
        ...
        # Add diagnostic logging before barrier
        logger.info(f"[Rank {self.rank}] Reaching barrier after model initialization...")
        ...
        # Add diagnostic logging before FSDP initialization
        logger.info(f"[Rank {self.rank}] Starting FSDP initialization...")
        torch.distributed.barrier()
        logger.info(f"[Rank {self.rank}] All ranks reached FSDP initialization barrier")
        ...
        if fsdp_strategy == "fsdp":
            # Add diagnostic logging before FSDP wrapping
            logger.info(f"[Rank {self.rank}] About to wrap model with FSDP, strategy={sharding_strategy}, cpu_offload={cpu_offload}")
            ...
            # Add diagnostic logging after FSDP wrapping
            logger.info(f"[Rank {self.rank}] FSDP wrapping completed")
            torch.distributed.barrier()
            logger.info(f"[Rank {self.rank}] All ranks passed FSDP wrapping barrier")
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        # When param offload is enabled, rollout generation must load FSDP params
        # back to the compute device before summon_full_params() in HFRollout.
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during generate_sequences", logger=logger)
