...
# The ``gigpo`` and ``agent_system`` modules were part of the upstream
# verl-agent fork and are not used by ARISE. Import them lazily so the trainer
# still loads in pure-text RL setups; users who need the multi-turn agent path
# or the GiGPO advantage estimator must reinstate the corresponding packages.
try:
    from gigpo import core_gigpo  # type: ignore  # noqa: F401
except ImportError:  # pragma: no cover - optional dependency
    core_gigpo = None  # type: ignore

try:
    from agent_system.multi_turn_rollout import (  # type: ignore  # noqa: F401
        TrajectoryCollector,
        adjust_batch,
    )
except ImportError:  # pragma: no cover - optional dependency
    TrajectoryCollector = None  # type: ignore
    adjust_batch = None  # type: ignore
...
class ResourcePoolManager:
    ...
    def create_resource_pool(self):
        ...
        for resource_pool_name, process_on_nodes in self.resource_pool_spec.items():
            # Normalize: Ray requires non-empty list of positive counts (no zeros)
            try:
                normalized = [max(1, int(c) if c is not None else 1) for c in (process_on_nodes or [])]
            except (TypeError, ValueError):
                normalized = [1]
            if not normalized:
                normalized = [1]
            normalized_spec[resource_pool_name] = normalized
            # max_colocate_count means the number of WorkerGroups (i.e. processes) in each RayResourcePool
            # For FSDP backend, we recommend using max_colocate_count=1 that merge all WorkerGroups into one.
            # For Megatron backend, we recommend using max_colocate_count>1
            # that can utilize different WorkerGroup for differnt models
            resource_pool = RayResourcePool(process_on_nodes=normalized, use_gpu=True, max_colocate_count=1, name_prefix=resource_pool_name)
            self.resource_pool_dict[resource_pool_name] = resource_pool

        self.resource_pool_spec = normalized_spec
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, gigpo_mode="mean_std_norm", gigpo_enable_similarity=False, gigpo_similarity_thresh=0.95, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if core_gigpo is None:
            raise RuntimeError(
                "AdvantageEstimator.GiGPO selected but the `gigpo` package "
                "is not installed. Install the upstream verl-agent fork or "
                "switch the advantage estimator (e.g. GRPO / DAPO)."
            )
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        device_name="cuda",
        traj_collector: Optional[Any] = None,  # TrajectoryCollector when agent_system is installed
        envs=None,
        val_envs=None,
    ):
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # Carry reward-related fields from the original batch into the
            # generation output so that the reward function can access them.
            for key in ("reward_model", "extra_info", "ground_truth", "answer"):
                if key in test_batch.non_tensor_batch and key not in test_output_gen_batch.non_tensor_batch:
                    test_output_gen_batch.non_tensor_batch[key] = test_batch.non_tensor_batch[key]
            del test_batch
            ...
            if 'tool_callings' in test_output_gen_batch.non_tensor_batch:
                tool_calling_list.append(test_output_gen_batch.non_tensor_batch['tool_callings'])
            else:
                batch_len = test_output_gen_batch.batch["responses"].shape[0]
                tool_calling_list.append([0] * batch_len)
            if 'traj_uid' in test_output_gen_batch.non_tensor_batch:
                traj_uid_list.append(test_output_gen_batch.non_tensor_batch['traj_uid'])
            else:
                batch_len = test_output_gen_batch.batch["responses"].shape[0]
                traj_uid_list.append([f"val-{i}" for i in range(batch_len)])
            # success rate
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        if self.traj_collector is not None:
                            self.traj_collector.update_skill_stats(batch, actor_rollout_wg=self.actor_rollout_wg)
