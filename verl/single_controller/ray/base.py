...
class RayResourcePool(ResourcePool):
    def __init__(
        ...
        # Ensure non-empty and no zero counts (Ray placement_group requires non-empty bundles)
        if process_on_nodes:
            process_on_nodes = [max(1, int(c) if c is not None else 1) for c in process_on_nodes]
        else:
            process_on_nodes = [1]
    ...
    def get_placement_groups(self, strategy="STRICT_PACK", name=None, device_name="cuda"):
        ...
        # Ray requires resource keys to be strings (e.g. "GPU" / "NPU").
        # DAPO may pass device_name=None, so normalize here defensively.
        ...
        # Ensure no empty bundles (Ray requires non-empty list per placement group)
        try:
            raw_store = list(self._store) if self._store else []
            store = [max(1, int(c) if c is not None else 1) for c in raw_store]
        except (TypeError, ValueError):
            store = [1]
        store = [x for x in store if x > 0]
        if not store:
            store = [1]
        # Each placement group must have at least one bundle; use max(1, pc) in case pc is 0
        pg_scheme = [[bundle.copy() for _ in range(max(1, int(pc)))] for pc in store]

        lifetime = "detached" if self.detached else None

        if not pg_scheme:
            pg_scheme = [[bundle.copy()]]

        # Build placement groups one-by-one; never pass empty bundles (Ray rejects it)
        pgs = []
        for idx, bundles in enumerate(pg_scheme):
            if not bundles:
                import logging
                logging.warning(
                    "get_placement_groups: empty bundles at idx=%s, _store=%s, store=%s, pg_scheme_lens=%s; using one bundle.",
                    idx, getattr(self, "_store", None), store, [len(b) for b in pg_scheme],
                )
                bundles = [bundle.copy()]
            pgs.append(placement_group(bundles=bundles, strategy=strategy, name=pg_name_prefix + str(idx), lifetime=lifetime))
